...
if prompt := st.chat_input("Type your issue here..."):
    ...
    # Check if a password key exists in active memory
    if "TEMPORARY_GEMINI_KEY" in st.session_state and st.session_state["TEMPORARY_GEMINI_KEY"]:
        try:
            ...
            # If key fails or expires, run backup mock rules
        ...
        if "water" in prompt_lower or "pipe" in prompt_lower or "leak" in prompt_lower:
            ...
            assigned_urgency = 6
        elif "power" in prompt_lower or "current" in prompt_lower or "electricity" in prompt_lower:
            assigned_cat = "Power Outage"
            assigned_urgency = 8
        elif "waste" in prompt_lower or "garbage" in prompt_lower or "trash" in prompt_lower:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO complaints (id, category, urgency_score, summary, status) VALUES (?, ?, ?, ?, ?)",
        (tracking_id, assigned_cat, assigned_urgency, prompt, "Pending")
    )
    conn.commit()
    conn.close()
    
    with st.chat_message("assistant"):
        ai_response = f"Grievance filed! Reference ID: **{tracking_id}**. Classified under **{assigned_cat}** with an Urgency rating of **{assigned_urgency}/10**.\n\n*({ai_mode_notice})*"
